...
The `/api/cron/prospecting-hubspot` job runs daily during the 5 p.m. hour in America/Chicago. Two once-daily UTC schedules cover both daylight-saving offsets; the handler accepts only the 5 p.m. hour. On Vercel Hobby, invocation can occur anywhere from 5:00 to 5:59 p.m.; schedules that run more than once daily are rejected during deployment. The job processes the queue as it stood at 5 p.m.; later stage entries wait for the next day. Failed or unfinished exports remain queued for the next day's run or a manual push. Successful exports are skipped, and manual/automatic pushes share a per-lead lease. HubSpot IDs are saved as each step completes, and failures are recorded on the lead and in `cron_run_log`. The two recurring-order schedules are independent and unchanged.

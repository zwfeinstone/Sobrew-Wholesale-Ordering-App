create table if not exists sales_prospecting_blocks (
  id uuid primary key default gen_random_uuid(),
  activity_date date not null,
  block_label text,
  calls_no_contact integer not null default 0 check (calls_no_contact >= 0),
  calls_voicemail integer not null default 0 check (calls_voicemail >= 0),
  calls_email integer not null default 0 check (calls_email >= 0),
  calls_contact integer not null default 0 check (calls_contact >= 0),
  calls_text integer not null default 0 check (calls_text >= 0),
  samples_from_contact integer not null default 0 check (samples_from_contact >= 0),
  samples_from_voicemail_callback integer not null default 0 check (samples_from_voicemail_callback >= 0),
  samples_from_email_reply integer not null default 0 check (samples_from_email_reply >= 0),
  samples_from_text_reply integer not null default 0 check (samples_from_text_reply >= 0),
  samples_other integer not null default 0 check (samples_other >= 0),
  notes text,
...
alter table sales_prospecting_blocks enable row level security;

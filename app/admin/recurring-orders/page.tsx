...
export default async function AdminRecurringOrdersPage({ searchParams }: { searchParams: Record<string, string | string[] | undefined> }) {
  ...
  let recurringQuery = supabase
    .from('recurring_orders')
    .select('id,user_id,source_order_id,frequency,status,amount_cents,created_at,last_generated_at,profiles(email,full_name)')
    .neq('status', 'canceled')
  ...
  if (recurringResult.error) {
    ...
    if (hasUnsupportedStatusFilter) {
      ...
      let legacyQuery = supabase
        .from('recurring_orders')
        .select('id,user_id,source_order_id,frequency,active,amount_cents,created_at,last_generated_at,profiles(email,full_name)')
        .order('created_at', { ascending: false });
  ...
  let normalizedItems: Array<{ id: string; recurring_order_id: string; product_id?: string | null; product_name_snapshot?: string | null; qty: number }> =
    (recurringItemsResult.data ?? []) as any[];

  if (recurringItemsResult.error) {
    const sourceOrderIds = recurringOrders.map((order: any) => order.source_order_id).filter(Boolean) as string[];
    const sourceItemsResult = sourceOrderIds.length
      ? await supabase
          .from('order_items')
          .select('id,order_id,product_id,product_name_snapshot,qty')
    ...
    if (sourceItemsResult.error) {
      ...
      const recurringOrderIdBySourceOrderId = new Map(recurringOrders.map((order: any) => [order.source_order_id, order.id]));
      normalizedItems = (sourceItemsResult.data ?? []).map((item: any) => ({
        ...
        qty: item.qty
      })).filter((item: any) => Boolean(item.recurring_order_id));
    }
  }

  const productIds = [...new Set((normalizedItems ?? []).map((item: any) => item.product_id).filter(Boolean))];

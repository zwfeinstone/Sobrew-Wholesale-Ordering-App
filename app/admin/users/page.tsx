...
import { getAssignedCenterIdsForAdmin, scopeCenterRelatedQueryForAdmin, scopeCentersForAdmin } from '@/lib/admin-center-scope';
...
export default async function UsersPage() {
  ...
  const centerMembersQuery = scopeCenterRelatedQueryForAdmin(
    supabase.from('profiles').select('id,center_id,is_active').eq('is_admin', false).not('center_id', 'is', null),
    'center_id',
    centerScope
  );
  const orderActivityQuery = scopeCenterRelatedQueryForAdmin(
    supabase.from('orders').select('center_id,user_id,created_at,notes').order('created_at', { ascending: false }).limit(10000),
    'center_id',
    centerScope
  );
